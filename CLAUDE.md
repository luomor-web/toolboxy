...
每个工具是一个自包含的 `<name>.html`（内联 CSS/JS + 引用 `form.css`），按后端需求分三类：

1. **纯前端**（大多数，如 json.html、base64.html、aes.html）——全部逻辑在页尾内联 `<script>`，本地运行不上传数据。
2. **前端 + 静态 JSON**（youbian/diqu/jichang/guojia/sfz/shouji/tld/chepai/rili）——JS `fetch('data/xxx.json')` 加载 `data/` 下手工整理的数据文件，数据变更无需改代码。
3. **PHP 后端**：
   - `pinyin.html → pinyin.php`、`zidian.html → zidian.php`：表单 POST 到 PHP，PHP 直接渲染结果页（同一文件既处理请求又输出 HTML）。
   - `dns.html → dns.php`：JS fetch JSON API（PHP 原生 `dns_get_record`）。
   - `rili.html → proxy.php?url=`：远程 .ics/JSON 代理，带 SSRF 防护（拒绝内网 IP）。
...
- **`lib.php`** — 从主站复制的 `xx_*` 函数库（笔顺加载、田字格渲染等），目前仅 `zidian.php` 使用。注意第 3 行 `include` 同目录 `Pinyin.php`，但该文件不在本仓库中（依赖部署环境从主站复制）；`pinyin.php` 页面则用 `../Pinyin.php`（本仓库父目录存在）。
...
页面结构高度模板化，复制现有工具页（如 `json.html`）最快。需要同步修改的位置：

1. 新建 `<name>.html`（含 canonical `https://toolboxy.luomor.com/<name>.html`、keywords/description meta、AdSense 脚本块原样保留）
2. `index.html` 加工具卡片
3. **每个页面的 `.nav-bar > .nav-grid` 都要加链接**（导航 HTML 在全部 48 页中逐页重复，需批量同步；可用 sed 批量插入）
4. `sitemap.xml` 加条目
